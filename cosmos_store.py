...
class CosmosVectorStore:
    ...
    def add_embeddings(self, embeddings: np.ndarray, metadatas: List[Dict]) -> Tuple[int, int]:
        """Add embeddings to Cosmos DB - FAST PARALLEL VERSION"""
    ...
    def _fallback_search(self, query_vec: np.ndarray, top_k: int = 5) -> List[Tuple[float, Dict]]:
        ...
        logger.info(f"🔍 Fallback: Batch fetching embeddings...")

        # Batch fetch all embeddings
        query = "SELECT c.id, c.embedding FROM c WHERE c.is_latest = true"
        ...
        try:
            ...
            for doc in all_docs:
                ...
                similarities.append((similarity, doc['id']))
